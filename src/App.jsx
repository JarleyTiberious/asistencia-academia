...
export default function App() {
  ...
  const nivelesAcademia = ["Primaria", "ESO", "Bachillerato", "PAU", "Mayores 25", "Acceso Grado", "B1", "B2", "C1", "APTIS"];
  const horariosAcademia = ["16:15-17:15", "17:15-18:15", "19:15-20:15", "20:15-21:15"];
  const opcionesDias = [
  ...
  const procesarComandoVoz = (frase) => {
    ...
    if (alumnoEncontrado) {
      let estadoVoz = null;
      if (texto.includes("presente") || texto.includes("vino") || texto.includes("asiste")) {
        estadoVoz = "PRESENTE";
      } else if (texto.includes("ausente") || texto.includes("falta") || texto.includes("no vino")) {
        estadoVoz = "AUSENTE";
      } else if (texto.includes("recupera")) {
        estadoVoz = "RECUPERACIÓN";
      }
  ...
  const exportarExcelMensual = () => {
    ...
    const filas = alumnosDelProfesor.map(alumno => {
      const filaAlumno = [alumno.nombre, alumno.grupo, alumno.horario || '', alumno.dias || ''];
      arregloDias.forEach(dia => {
        ...
        if (!registroEncontrado && alumno.horario) {
          ...
          if (asistencias[claveEspecial]) registroEncontrado = asistencias[claveEspecial];
        }
        let estadoStr = "-";
        if (registroEncontrado?.estado === 'PRESENTE') estadoStr = "PRESENTE";
        if (registroEncontrado?.estado === 'AUSENTE') estadoStr = "AUSENTE";
        if (registroEncontrado?.estado === 'RECUPERACIÓN') estadoStr = "RECUPERACION";
    ...
    XLSX.writeFile(libro, `Asistencia_${profesorActivo}_${nombreMes}_${anioCuadrante}.xlsx`);
  };

  // --- FILTROS Y ARREGLOS DE DÍAS ---
  ...
  const alumnosFiltrados = alumnos.filter(a => {
    ...
    const tieneAsistenciaAquiHoy = asistencias[`${fechaSeleccionada}_${horaSeleccionada}_${a.id}`];
    return (coincideHora && coincideDia) || tieneAsistenciaAquiHoy;
  });

  const alumnosDeOtrasClases = alumnos.filter(a => 
    a.profesor === profesorActivo && 
    a.horario && a.dias &&
    (a.horario !== horaSeleccionada || (a.dias !== diaFiltroTabla && a.dias !== 'L-M-X-J-V'))
  );
          ...
          {vistaAdmin ? (
            ...
            <>
              ...
              <div style={tarjetaEstilo}>
                ...
                {/* FILTROS DÍAS LECTIVOS Y RECUPERACIONES */}
                <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '15px', marginBottom: '15px', padding: '10px 15px', backgroundColor: '#0f172a', borderRadius: '8px', border: '1px solid #334155' }}>
                  ...
                  <div>
                    <select
                      onChange={(e) => {
                        if (e.target.value) {
                          marcarAsistencia(e.target.value, 'RECUPERACIÓN');
                          e.target.value = ''; 
                        }
                      }}
                      style={{ ...inputEstilo, padding: '6px 10px', fontSize: '12px', fontWeight: 'bold', borderColor: '#3b82f6' }}
                    >
                      <option value="">🔄 Registrar Alumno de otra clase (Recuperar)</option>
                      {alumnosDeOtrasClases.map(al => (
                        <option key={al.id} value={al.id}>
                          {al.nombre} ({al.grupo} — {al.dias} {al.horario})
                        </option>
                      ))}
                    </select>
                  </div>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                    <thead>
                      <tr style={{ borderBottom: '2px solid #334155', color: '#94a3b8', fontSize: '13px' }}>
                        ...
                        <th style={{ padding: '12px', textAlign: 'center' }}>Horario Base</th>
                        <th style={{ padding: '12px', textAlign: 'center' }}>Días Base</th>
                        <th style={{ padding: '12px', textAlign: 'center' }}>Acciones de Asistencia</th>
                          ...
                          <tr key={a.id} style={{ borderBottom: '1px solid #334155' }}>
                            ...
                            <td style={{ padding: '12px' }}>
                              <div style={{ display: 'flex', gap: '6px', justifyContent: 'center' }}>
                                <button onClick={() => marcarAsistencia(a.id, 'PRESENTE')} style={{ backgroundColor: historial?.estado === 'PRESENTE' ? '#16a34a' : '#0f172a', color: historial?.estado === 'PRESENTE' ? 'white' : '#4ade80', border: '1px solid #16a34a', padding: '6px 10px', borderRadius: '6px', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer' }}>✓ PRESENTE</button>
                                <button onClick={() => marcarAsistencia(a.id, 'AUSENTE')} style={{ backgroundColor: historial?.estado === 'AUSENTE' ? '#dc2626' : '#0f172a', color: historial?.estado === 'AUSENTE' ? 'white' : '#f87171', border: '1px solid #dc2626', padding: '6px 10px', borderRadius: '6px', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer' }}>✕ AUSENTE</button>
                                <button onClick={() => marcarAsistencia(a.id, 'RECUPERACIÓN')} style={{ backgroundColor: historial?.estado === 'RECUPERACIÓN' ? '#2563eb' : '#0f172a', color: historial?.estado === 'RECUPERACIÓN' ? 'white' : '#60a5fa', border: '1px solid #2563eb', padding: '6px 10px', borderRadius: '6px', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer' }}>🔄 RECUPERA</button>
                              </div>
                            </td>

                            <td style={{ padding: '12px', textAlign: 'center', fontSize: '12px' }}>
                              {historial ? (
                                <span style={{ fontWeight: 'bold', color: historial.estado === 'PRESENTE' ? '#4ade80' : historial.estado === 'AUSENTE' ? '#f87171' : '#60a5fa' }}>{historial.estado}</span>
                              ) : <span style={{ color: '#475569' }}>-</span>}
                            </td>

                            <td style={{ padding: '12px', textAlign: 'center' }}>
                              <button onClick={() => eliminarAlumno(a.id)} style={{ backgroundColor: 'transparent', border: 'none', color: '#ef4444', fontSize: '16px', cursor: 'pointer' }} title="Eliminar alumno">🗑️</button>
                            </td>
                          </tr>
                        );
                      })}
              ...
              <div style={tarjetaEstilo}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #334155', paddingBottom: '15px', marginBottom: '15px', flexWrap: 'wrap', gap: '15px' }}>
                  <div>
                    <h2 style={{ fontSize: '18px', fontWeight: 'bold', margin: '0 0 5px 0' }}>📅 Cuadrante de Asistencia Mensual</h2>
                    <p style={{ color: '#94a3b8', fontSize: '12px', margin: 0 }}>Muestra las asistencias e incluye recuperaciones en azul</p>
                  </div>
                  
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                    <button onClick={exportarExcelMensual} style={botonVerde}>
                      ⬇️ Descargar Excel
                    </button>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                    <thead>
                      <tr style={{ backgroundColor: '#0f172a', color: '#94a3b8' }}>
                        ...
                        {arregloDias.map(dia => (
                          <th key={dia} style={{ padding: '5px 2px', textAlign: 'center', minWidth: '24px', border: '1px solid #334155', backgroundColor: '#1e293b' }}>{dia}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {alumnosDelProfesor.map(alumno => (
                        <tr key={alumno.id} style={{ borderBottom: '1px solid #334155' }}>
                          ...
                          {arregloDias.map(dia => {
                            ...
                            if (registroEncontrado?.estado === 'RECUPERACIÓN') renderEstado = "🔵";
